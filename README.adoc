...
=== Data Streamer
Before each device is able to send data to the application it needs to communicate with the mobile network to ensure that the data is allowed to be send over the network. This communication happens through signalling events which is usually hidden from the application.
...
The Data Streamer enables real-time streaming of connectivity data into the cloud,  and self-developed software stacks. These integrations empower you with complete network transparency to faster triage and resolve issues. 

This integrations reveals a wealth of insights for IoT remote monitoring and anomaly detection or providing detailed information about service (data, SMS) usage and cost per device.

Additionaly, data streams can be configured to contain also events generated by the EMnify plattform when your users perform  billing related updates, order SIMs and many other operations. 
...
==== Events
The EMnify platform generates different types of events and we regulary add new events types. Events are often used as triggers for custom business processes, for monitoring and as input for custom billing systems. They provide information about lifecycle transitions and config changes. You may distinguish between normal/expected, unusual and critical events through the event severity (INFO, WARN, CRITICAL).
...
• Limit enforcement events (cost/volume limits, prepaid volume, endpoint quota lifecycle events such as quota threshold
reached/quota used up, organisation wide or endpoint specific)
• Platform authentication events (user/application authentication failed, support access granded, MFA activated/deactivated,
password reset requested, …)
• CloudConnect lifecycle events (resource-share created, breakout available, breakout terminated, state changed; for TGW, VPN,
IpSec, …)
• OpenVPN lifecycle events (authentication, connected/disconnected)
Organisation and billing config related events (Self-Signup performed, new user invited, billing config updated, organisation data
updated, payment could not be settled, credit card will expire, …)
• Usage Tariff and Tariff Plan related events (Usage Tariff changed, end of evaluation period reached, inclusive volume
booked/extended, … )
• SIM order events

Please visit the EMnify System Documentation (https://api-docs.dev.emnify.io/event.html) to learn more about the event structure.

==== Usage Data
Usage data records get generated when endpoints consumes data or SMS services. They provide information about the used service, visited network, volume and cost. Usage data streams are often used for monitoring and analysing the data consumption and as input for custom billing system.
...
Multiple independent data streams using any connection type can be created and they will operate in parallel. Each data stream can
be configured to push data to a separate platform for consumption and processing; For example, one stream may be connected to a
network monitoring system, another can connect to data analytics platform and another can be syncing with S3 for archiving at the
same time.

Following connection types are currenly available::
...
Data Streamer can be managed by using the EMnify Portal or by using the EMnify API.
Portal::
Managing data steams via the Portal is convenient when you are taking the first steps or if you do not expect configuration changes being needed often.

ADD IMAGE

API::
The integration of the Data Streamer API becomes a faster and more secure approach when multiple data streams need to be managed as part of the daily business. It provides you a seamless integration into yout own applications.
